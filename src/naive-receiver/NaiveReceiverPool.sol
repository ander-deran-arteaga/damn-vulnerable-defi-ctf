...
contract NaiveReceiverPool is Multicall, IERC3156FlashLender {
    ...
    function flashLoan(IERC3156FlashBorrower receiver, address token, uint256 amount, bytes calldata data)  // error can be here
        external
        returns (bool)
    {
        if (token != address(weth)) revert UnsupportedCurrency();
    ...
    function withdraw(uint256 amount, address payable receiver) external { // a: not only Owner!!!!
        // Reduce deposits
